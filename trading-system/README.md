...
| ⑤ 자동화 & 알림 | `bash scripts/install_cron.sh --apply` | 매일 자동 실행 + 폰 알림 |
...
python3 -m pytest tests/ -q                            # 테스트 54개
```
...
| 스캐너 | 실행 시간 (ET) |
|---|---|
| 프리마켓 갭 (A) | 오전 8:30 ~ 오후 2:00, 30분 간격 |
| 전략 (B) | 오전 10:05 ~ 오후 3:05, 매시간 |

> ⚠️ **컴퓨터가 켜져 있고 잠들지 않아야 실행됩니다.** 노트북을 덮으면 그 시간 스캔은
> 건너뜁니다. 매일 빠짐없이 받고 싶으면 상시 켜두는 PC 나 클라우드 서버가 필요합니다.
> macOS 는 `caffeinate`, Windows 는 작업 스케줄러 + 절전 해제 설정을 함께 보세요.
