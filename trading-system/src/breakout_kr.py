...
def report(hits: list[Hit], cfg: Setup, top: int = 20) -> str:
    ...
    lines.append("")
    lines.append("[사실] 신호가 났을 때 확인한 것 — 재 본 숫자입니다")
    lines.append("   · 다음날 아침 5% 넘게 갭 상승해서 시작한 건은 5일 뒤 평균 -4.2%,")
    lines.append("     중앙값 -5.9%, 승률 30% 였습니다(155건). 갭이 크면 그날은")
    lines.append("     사지 않는 편이 낫습니다. 시가는 아침에 보고 정하면 됩니다.")
    lines.append("   · 손절 3% 는 진입 첫날에 46.8% 가 닿았습니다. 8% 면 6.0%.")
    lines.append("     좁은 손절은 신호가 맞든 틀리든 절반을 먼저 잘라냅니다.")
    lines.append("=" * 86)
    return "\n".join(lines)
