...
# ── 왕복 거래비용 ──
#
# 2026-09-03, 사장님이 KB금융 1주를 실제로 사고 파신 명세로 맞췄습니다.
# (매수 168,200원 → 매도 178,700원, 매매비용 405원, 실현손익 +10,095원)
#
...
#   요율 합계                     0.2274%   ← 확인된 값
#
# ⚠️ 이 거래에서 실제로 떼인 405원은 매수금액의 0.2408% 입니다. 요율
# 합계(0.2274%)보다 큰 이유는 **6% 오른 뒤에 팔아서 매도 쪽 기준금액이
# 그만큼 컸기** 때문입니다. 백테스트는 진입가 기준 수익률에서 비용을
# 빼므로, 오르든 내리든 통하는 요율 합계를 씁니다. 이익이 클수록 실제
# 비용은 이보다 조금 더 나갑니다.
#
# 슬리피지(사려던 값과 실제 체결가의 차이)는 이 명세에 안 나옵니다.
# 왕복 0.30% 로 두던 가정을 그대로 씁니다 — **아직 확인 못 한 값입니다.**
# 대형주는 이보다 작고, 거래 적은 소형주는 훨씬 큽니다.
COST_FEES_PCT = 0.2274       # 확인됨 (2026-09-03, KB금융 1주 거래명세)
COST_SLIPPAGE_PCT = 0.30     # 가정 — 확인 못 함
COST_ROUND_TRIP_PCT = round(COST_FEES_PCT + COST_SLIPPAGE_PCT, 4)   # 0.5274
...
def exit_grid(paths: Paths, stops: tuple[float, ...] = STOPS,
              ...
              cost_pct: float = COST_ROUND_TRIP_PCT) -> pd.DataFrame:
    ...
    비용은 확인된 수수료·세금 0.2274% 에 슬리피지 가정 0.30% 를 더한 값입니다.
    수수료·세금은 실제 거래명세로 맞췄고, 슬리피지는 아직 확인 못 했습니다.
    """
    if len(paths) == 0:
        return pd.DataFrame()
...
def report(curve: list[HoldRow], grid: pd.DataFrame, missed: dict,
    ...
    줄 += [f"② 손절폭과 익절 목표를 바꾸면 "
           f"(비용 왕복 {COST_ROUND_TRIP_PCT:g}% 뺀 값 — 수수료·세금 "
           f"{COST_FEES_PCT:g}% 확인됨 + 슬리피지 {COST_SLIPPAGE_PCT:g}% 가정)", ""]
