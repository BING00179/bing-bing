...
class BacktestConfig:
    ...
    # ── 손절 방식 ──
    # 고정 퍼센트 손절은 종목마다 다른 변동성을 무시합니다. 하루에 6%
    # 씩 움직이는 종목에 3% 손절을 걸면 신호와 무관하게 절반이 그냥
    # 걸립니다. 실제로 매매의 46% 가 진입 당일 손절로 끝났습니다.
    #
    # atr_stop_mult 를 0 보다 크게 두면 손절폭을 그 종목의 ATR 에
    # 맞춥니다. 예: 2.0 이면 평소 하루 변동폭의 2배.
    # 이때 stop_loss_pct 는 하한으로만 쓰입니다.
    atr_stop_mult: float = 0.0        # 0 = 끔 (고정 % 손절)
    atr_window: int = 14
    atr_stop_cap_pct: float = 15.0    # 아무리 변동성이 커도 이 이상은 안 벌림
...
class BacktestKrConfig(BacktestConfig):
    ...
    수수료와 세금은 **실제 거래명세로 맞춘 값**입니다 (2026-09-03,
    KB금융 1주. 매수 168,200 → 매도 178,700, 매매비용 405원).

        구매수수료   23원 / 168,200 = 0.0137%
        판매수수료   25원 / 178,700 = 0.0140%
        세금        357원 / 178,700 = 0.1998%

    세금은 처음에 0.18% 로 잡고 있었는데 실제가 더 높았습니다. 요율은
    해마다 바뀌고 수수료는 증권사마다 다르므로, 본인 명세로 다시
    맞춰 주세요. 비용을 낮게 잡으면 백테스트가 실제보다 좋게 나옵니다.

    슬리피지만은 여전히 **가정**입니다. 사려던 값과 체결가의 차이는
    거래명세에 나오지 않습니다. 거래 적은 소형주는 이보다 훨씬 큽니다.
    """

    commission_per_trade: float = 0.0     # 국내는 정액이 아니라 정률
    commission_pct: float = 0.014         # 편도 수수료율 (%) — 확인됨
    sell_tax_pct: float = 0.20            # 매도 시 세금 (%) — 확인됨
    slippage_pct: float = 0.15            # 편도 슬리피지 (%) — 가정
