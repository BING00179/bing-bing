...
# ────────── 비용은 확인된 것과 가정한 것을 갈라 둔다 ──────────
#
# 2026-09-03, 사장님이 KB금융 1주를 실제로 사고 파신 명세로 맞췄습니다.
# 매수 168,200 → 매도 178,700, 매매비용 405원, 실현손익 +10,095원.

def test_수수료와_세금은_실제_거래명세와_맞는다():
    매수, 매도 = 168_200, 178_700
    구매수수료, 판매수수료, 세금 = 23, 25, 357
    요율합계 = (구매수수료 / 매수 + (판매수수료 + 세금) / 매도) * 100
    assert abs(ex.COST_FEES_PCT - 요율합계) < 0.001

    # 실제로 떼인 405원은 매수금액의 0.2408% 입니다. 요율 합계보다 큰 것은
    # 6% 오른 뒤에 팔아 매도 쪽 기준금액이 그만큼 컸기 때문입니다.
    실제로떼인비율 = 405 / 매수 * 100
    assert 실제로떼인비율 > 요율합계
    assert abs(실제로떼인비율 - 0.2408) < 0.001


def test_실현손익이_우리_계산과_맞는다():
    """비용을 뺀 뒤 남는 값이 증권사 화면과 같아야 합니다."""
    매수, 매도, 비용 = 168_200, 178_700, 405
    assert 매도 - 매수 - 비용 == 10_095
    assert abs((매도 - 매수 - 비용) / 매수 * 100 - 6.00) < 0.01


def test_확인된_비용과_가정한_비용을_갈라_둔다():
    """슬리피지는 거래명세에 안 나옵니다. 확인된 척하면 안 됩니다."""
    assert ex.COST_FEES_PCT < ex.COST_ROUND_TRIP_PCT
    assert abs(ex.COST_ROUND_TRIP_PCT
               - (ex.COST_FEES_PCT + ex.COST_SLIPPAGE_PCT)) < 1e-9
